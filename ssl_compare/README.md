...
`logs/` + `results/` + (optionally) `pretrain_base_<ds>_<ver>/` ckpt copies.
`results/ssl_compare_summary.csv` is the final table for each run; `tsne_compare.png`
(if present) is the embedding visualization for the same set of checkpoints.
